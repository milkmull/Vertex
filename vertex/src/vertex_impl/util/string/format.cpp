...
bool parse_format_string(const std::string& fmt, std::vector<format_token>& stack)
{
    ...
    for (size_t i = 0; i < size; ++i)
    {
        ...
        if (c == '{')
        {
            if (in_field)
            {
                VX_ERR(err::INVALID_ARGUMENT) << "invalid { at position " << i;
                return false;
            }
        ...
        if (c == '}')
        {
            if (!in_field)
            {
                ...
                VX_ERR(err::INVALID_ARGUMENT) << "unmatched } at position " << i;
                return false;
            }
        ...
        if (c == ':')
        {
            ...
            token->specifier_off = ++i;

            size_t j = i;
            while (j < size && fmt[j] != '}')
            {
                ++token->specifier_size;
                ++j;
            }
            i = j - 1;
        ...
        if (in_field)
        {
            VX_ERR(err::INVALID_ARGUMENT) << "invalid character " << c << " in field";
            return false;
        }
    }
    ...
    if (in_field)
    {
        VX_ERR(err::INVALID_ARGUMENT) << "missing closing bracket";
        return false;
    }
