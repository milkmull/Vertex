...
{
    VX_ERR(err::RESOURCE_ALREADY_EXISTS)
        << err::code_to_string(err::RESOURCE_ALREADY_EXISTS)
        << ": " << path;
}

static void file_operation_error(const std::string& path, const char* operation)
{
    VX_ERR(err::FILE_OPERATION_FAILED)
...
std::string get_user_folder(user_folder folder)
{
    ...
    {
        VX_ERR(err::RESOURCE_NOT_FOUND) << "could not find folder";
    }
...
#endif // VX_OS_WINDOWS_FILESYSTEM