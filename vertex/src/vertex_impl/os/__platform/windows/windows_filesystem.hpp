...
static file_info get_symlink_info_impl(const path& p)
{
    handle h = CreateFileW(
        p.c_str(),
...
static file_info get_file_info_impl(const path& p)
{
...
static size_t hard_link_count_impl(const path& p)
{
    handle h = CreateFileW(
        p.c_str(),
...
static bool set_modify_time_impl(const path& p, time::time_point t)
{
    handle h = CreateFileW(
        p.c_str(),
...
static path read_symlink_impl(const path& p)
{
    path symlink_path;
...
static bool set_current_path_impl(const path& p)
{
...
static path absolute_impl(const path& p)
{
...
static path canonical_impl(const path& p)
{
    path res;
...
static bool equivalent_impl(const path& p1, const path& p2)
{
...
static path get_user_folder_impl(user_folder folder)
{
    path p;
...
static bool create_file_impl(const path& p)
{
...
static bool create_directory_impl(const path& p)
{
...
static bool create_symlink_impl(const path& target, const path& link)
{
    return create_symlink_impl(target, link, 0);
}

static bool create_directory_symlink_impl(const path& target, const path& link)
{
    return create_symlink_impl(target, link, SYMBOLIC_LINK_FLAG_DIRECTORY);
}

static bool create_hard_link_impl(const path& target, const path& link)
{
#if defined(_CRT_APP)
...
static bool copy_file_impl(const path& from, const path& to, bool overwrite_existing)
{
...
static bool rename_impl(const path& from, const path& to)
{
...
static __detail::remove_error remove_impl(const path& p, bool in_recursive_remove)
{
...
static space_info space_impl(const path& p)
{
