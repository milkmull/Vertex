...
struct mat<2, 4, T>
{
    ...
    static VX_FORCE_INLINE constexpr size_t size() noexcept { return static_cast<size_t>(8); }
    ...
    VX_FORCE_INLINE constexpr mat(const vec<4, C1>& c1, const vec<4, C2>& c2)
        : columns{ static_cast<col_type>(c1),
                   static_cast<col_type>(c2) } {}

    template <typename U>
    VX_FORCE_INLINE constexpr mat(const mat<2, 4, U>& m)
        : columns{ static_cast<col_type>(m.columns[0]),
                   static_cast<col_type>(m.columns[1]) } {}

    template <size_t M, size_t N, typename U, typename std::enable_if<(M >= 2 && N >= 4), bool>::type = true>
    ...
    template <typename U>
    VX_FORCE_INLINE constexpr type& operator=(const mat<2, 4, U>& m)
    {
    ...
    template <typename U = T, typename std::enable_if<std::is_same<T, U>::value && type_traits::is_numeric<U>::value, bool>::type = true>
    friend VX_FORCE_INLINE constexpr vec<4, T> operator*(const type& m, const vec<2, T>& v)
    {
        return vec<4, T>(
    ...
    template <typename U = T, typename std::enable_if<std::is_same<T, U>::value && type_traits::is_numeric<U>::value, bool>::type = true>
    friend VX_FORCE_INLINE constexpr vec<4, T> operator*(const vec<2, T>& v, const type& m)
    {
        return vec<4, T>(
    ...
    template <typename U = T, typename std::enable_if<std::is_same<T, U>::value && type_traits::is_numeric<U>::value, bool>::type = true>
    friend VX_FORCE_INLINE constexpr type operator*(const type& m1, const mat<2, 2, T>& m2)
    {
        return type(
    ...
    template <typename U = T, typename std::enable_if<std::is_same<T, U>::value && type_traits::is_numeric<U>::value, bool>::type = true>
    friend VX_FORCE_INLINE constexpr mat<3, 4, T> operator*(const type& m1, const mat<3, 2, T>& m2)
    {
        return mat<3, 4, T>(
            (m1.columns[0].x * m2.columns[0].x) + (m1.columns[1].x * m2.columns[0].y),
            (m1.columns[0].y * m2.columns[0].x) + (m1.columns[1].y * m2.columns[0].y),
    ...
    template <typename U = T, typename std::enable_if<std::is_same<T, U>::value && type_traits::is_numeric<U>::value, bool>::type = true>
    friend VX_FORCE_INLINE constexpr mat<4, 4, T> operator*(const type& m1, const mat<4, 2, T>& m2)
    {
        return mat<4, 4, T>(
            (m1.columns[0].x * m2.columns[0].x) + (m1.columns[1].x * m2.columns[0].y),
            (m1.columns[0].y * m2.columns[0].x) + (m1.columns[1].y * m2.columns[0].y),
    ...
    template <typename U = T, typename std::enable_if<std::is_same<T, U>::value && type_traits::is_numeric<U>::value, bool>::type = true>
    VX_FORCE_INLINE constexpr type& operator*=(const mat<2, 2, T>& m)
    {
        (*this) = (*this) * m;
        return *this;
    }
