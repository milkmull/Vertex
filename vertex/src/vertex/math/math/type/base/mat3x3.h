...
struct mat<3, 3, T>
{
    ...
    static inline constexpr size_t size() noexcept { return static_cast<size_t>(9); }
    static inline constexpr size_t width() noexcept { return static_cast<size_t>(3); }
    ...
    template <typename U = T, typename std::enable_if<std::is_same<T, U>::value && type_traits::is_numeric<U>::value, bool>::type = true>
    friend inline constexpr vec<3, T> operator*(const type& m, const vec<3, T>& v)
    {
        return vec<3, T>(
    ...
    template <typename U = T, typename std::enable_if<std::is_same<T, U>::value && type_traits::is_numeric<U>::value, bool>::type = true>
    friend inline constexpr vec<3, T> operator*(const vec<3, T>& v, const type& m)
    {
        return vec<3, T>(
    ...
    template <typename U = T, typename std::enable_if<std::is_same<T, U>::value && type_traits::is_numeric<U>::value, bool>::type = true>
    friend inline constexpr mat<2, 3, T> operator*(const type& m1, const mat<2, 3, T>& m2)
    {
        return mat<2, 3, T>(
    ...
    template <typename U = T, typename std::enable_if<std::is_same<T, U>::value && type_traits::is_numeric<U>::value, bool>::type = true>
    friend inline constexpr mat<4, 3, T> operator*(const type& m1, const mat<4, 3, T>& m2)
    {
        return mat<4, 3, T>(
            (m1.columns[0].x * m2.columns[0].x) + (m1.columns[1].x * m2.columns[0].y) + (m1.columns[2].x * m2.columns[0].z),
            (m1.columns[0].y * m2.columns[0].x) + (m1.columns[1].y * m2.columns[0].y) + (m1.columns[2].y * m2.columns[0].z),
    ...
    friend inline constexpr vec<2, T> operator/(const type& m, const vec<3, T>& v)
    {
        return invert(m) * v;
    }

    template <typename U = T, typename std::enable_if<std::is_same<T, U>::value && type_traits::is_floating_point<U>::value, bool>::type = true>
    friend inline constexpr vec<2, T> operator/(const vec<3, T>& v, const type& m)
    {
        return v * invert(m);
    }
     ...
     */
    template <typename U = T, typename std::enable_if<std::is_same<T, U>::value&& type_traits::is_floating_point<U>::value, bool>::type = true>
    static inline constexpr type from_quat(const quat_t<T>& rotation)
    {
     ...
     */
    template <typename U = T, typename std::enable_if<std::is_same<T, U>::value&& type_traits::is_floating_point<U>::value, bool>::type = true>
    static inline constexpr quat_t<T> to_quat(const type& m)
    {
     ...
     */
    template <typename U = T, typename std::enable_if<std::is_same<T, U>::value && type_traits::is_floating_point<U>::value, bool>::type = true>
    static inline constexpr type make_scale(const vec<3, T>& scale)
    {
        return type(
            scale.x,
            static_cast<T>(0),
     ...
     */
    template <typename U = T, typename std::enable_if<std::is_same<T, U>::value && type_traits::is_floating_point<U>::value, bool>::type = true>
    static inline constexpr vec<3, T> get_scale(const type& m)
    {
        return vec<3, T>(
