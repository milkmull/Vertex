...
inline constexpr T mirror(T a)
{
    return (a >= static_cast<T>(0)) ? a : -(static_cast<T>(1) + a);
}

template <typename T, typename std::enable_if<std::is_arithmetic<T>::value, bool>::type = true>
inline constexpr T clamp_to_edge(T p, T size)
{
    return math::clamp(p, static_cast<T>(0), size - static_cast<T>(1));
}

template <typename T, typename std::enable_if<std::is_arithmetic<T>::value, bool>::type = true>
inline constexpr T clamp_to_border(T p, T size)
{
    return math::clamp(p, static_cast<T>(-1), size);
}

template <typename T, typename std::enable_if<std::is_arithmetic<T>::value, bool>::type = true>
inline constexpr T repeat(T p, T size)
{
    return math::mod(p, size);
}

template <typename T, typename std::enable_if<std::is_arithmetic<T>::value, bool>::type = true>
inline constexpr T mirrored_repeat(T p, T size)
{
    return (size - static_cast<T>(1)) - mirror(math::mod(p, static_cast<T>(2) * size) - size);
}

template <typename T, typename std::enable_if<std::is_arithmetic<T>::value, bool>::type = true>
inline constexpr T mirror_clamp_to_edge(T p, T size)
{
