...
/// to (use image_format::UNKNOWN for no reinterpretation).
...
)
{
    ...
    if (target_format != image_format::UNKNOWN)
    {
...
bool load_image(const char* filename, image_info& info, image_format target_format, std::vector<byte_type>& data, bool flip_vertically_on_load)
{
