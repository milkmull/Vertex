...
int cursor_data::s_cursor_id = static_cast<int>(cursor::shape::SHAPE_CUSTOM) + 1;
std::unordered_map<int, cursor_data> cursor_data::s_cursor_cache = std::unordered_map<int, cursor_data>();

static void load_system_cursor(cursor::shape shape)
{
    ...
    : m_shape(shape), m_id(shape)
{
    ...
    if (shape != shape::SHAPE_CUSTOM && cursor_data::s_cursor_cache.find(static_cast<int>(shape)) == cursor_data::s_cursor_cache.end())
    {
...
cursor cursor::make_custom(const uint8_t* pixels, const math::vec2i& size, const math::vec2i& hotspot)
{
    ...
    cursor_data::s_cursor_cache[id].cursor = cursor;

    // Create the cursor
    ::vx::app::cursor custom_cursor(shape::SHAPE_CUSTOM);
...
cursor::shape cursor::get_shape() const { return m_shape; }
int cursor::get_id() const { return m_id; }
