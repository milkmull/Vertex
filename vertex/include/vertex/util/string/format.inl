...
struct format_token
{
    inline format_token(token_type type, size_t off, size_t size)
        : type(type), off(off), size(size) {}

    inline format_token(token_type type, size_t off, size_t size, const char* out)
...
{
    std::vector<__detail::format_token> tokens;
    if (!__detail::parse_format_string(fmt, tokens))
    {
        return fmt;
    }

    std::ostringstream oss;
    __detail::format_args<0>(fmt, tokens, oss, std::forward<Args>(args)...);
