...
public:
    ...
    /**
     * @brief Computes the SHA-256 hash of a data buffer_type in one call.
     *
     * @param data Pointer to the input data buffer_type.
     * @param size Size of the buffer_type in bytes.
...
private:
    ...
    uint64_t m_bit_count;                   // Total number of bits in the buffer_type
    uint8_t m_buffer[64];                   // Input buffer_type
    size_t m_buffer_size;                   // Current buffer_type size
