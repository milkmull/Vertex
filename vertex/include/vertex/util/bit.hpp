...
VX_FORCE_INLINE T rotl(T x, unsigned int shift)
{
    ...
    constexpr unsigned int mask = (CHAR_BIT * sizeof(T) - 1);
...
VX_FORCE_INLINE T rotr(T x, unsigned int shift)
{
    ...
    constexpr unsigned int mask = (CHAR_BIT * sizeof(n) - 1);
...
VX_FORCE_INLINE int countl_zero(T x)
{
    ...
    constexpr T mask = static_cast<T>(1) << (sizeof(T) - 1);
...
VX_FORCE_INLINE int bit_ceil(T x)
{
    ...
    return static_cast<T>(1) << bit_width(x);
}
