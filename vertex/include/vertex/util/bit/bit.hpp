...
/**
 * @brief Computes the index of the most significant set bit (floor(log2(x))).
 *
 * This generic fallback implementation is defined in terms of `bit_width`,
 * i.e. `bit_width(x) - 1`.
 *
 * @tparam T An unsigned integral type.
 * @param x The value to analyze. Must be nonzero.
 * @return The zero-based index of the highest set bit in `x`.
 *
 * @note The result is unspecified if `x == 0`, mirroring the behavior of
 * platform intrinsics such as `_BitScanReverse` and `__builtin_clz`.
 * @note Requires T to be an unsigned integral type.
 */
template <typename T, VX_REQUIRES(std::is_integral<T>::value&& std::is_unsigned<T>::value)>
constexpr int bit_log2(T x) noexcept
{
    return bit_width(x) - 1;
}

/**
 * @brief Computes the index of the most significant set bit in a 32-bit unsigned integer.
 *
 * Uses platform intrinsics when available:
 * - GCC/Clang: `__builtin_clz`
 * - MSVC: `_BitScanReverse`
 * Falls back to the generic implementation otherwise.
 *
 * @param x The 32-bit unsigned integer. Must be nonzero.
 * @return The zero-based index of the highest set bit. Unspecified if `x == 0`.
 */
constexpr int bit_log2(uint32_t x) noexcept
{
#if VX_HAS_BUILTIN(__builtin_clz)
...
/**
 * @brief Computes the index of the most significant set bit in a 64-bit unsigned integer.
 *
 * Uses platform intrinsics when available:
 * - GCC/Clang: `__builtin_clzll`
 * - MSVC: `_BitScanReverse64`
 * Falls back to the generic implementation otherwise.
 *
 * @param x The 64-bit unsigned integer. Must be nonzero.
 * @return The zero-based index of the highest set bit. Unspecified if `x == 0`.
 */
constexpr int bit_log2(uint64_t x) noexcept
{
#if VX_HAS_BUILTIN(__builtin_clzll)
