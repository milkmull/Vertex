...
template <typename T, typename U = typename type_traits::__detail::inv_unwrap<T>::type>
...
constexpr Res invoke_impl(type_traits::__detail::invoke_other, Fn&& fn, Args&&... args)
{
    return std::forward<Fn>(fn)(std::forward<Args>(args)...);
}

template <typename Res, typename MemFn, typename T, typename... Args>
constexpr Res invoke_impl(type_traits::__detail::invoke_memfun_ref, MemFn&& fn, T&& t, Args&&... args)
{
    return (invfwd<T>(t).*Fn)(std::forward<Args>(args)...);
}

template <typename Res, typename MemFn, typename T, typename... Args>
constexpr Res invoke_impl(type_traits::__detail::invoke_memfun_deref, MemFn&& fn, T&& t, Args&&... args)
{
    return ((*std::forward<T>(t)).*fn)(std::forward<Args>(args)...);
}

template <typename Res, typename MemPtr, typename T>
constexpr Res invoke_impl(type_traits::__detail::invoke_memobj_ref, MemPtr&& fn, T&& t)
{
    return invfwd<T>(t).*fn;
}

template <typename Res, typename MemPtr, typename T>
constexpr Res invoke_impl(type_traits::__detail::invoke_memobj_deref, MemPtr&& fn, T&& t)
{
...
invoke(Callable&& fn, Args&&... args) noexcept(type_traits::is_nothrow_invocable<Callable, Args...>::value)
{
    ...
    return type_traits::__detail::invoke_impl<T>(
        tag{},
