...
template <typename T, VX_MATH_REQ((is_float<T>::value && !simd::vec<4, T>::HAVE_SQRT))>
...
template <typename T, VX_MATH_REQ((is_float<T>::value && !simd::vec<4, T>::HAVE_INVERSE))>
...
template <typename T, VX_MATH_REQ((is_float<T>::value && !simd::vec<4, T>::HAVE_INVERSE_SQRT))>
...
VX_FORCE_INLINE constexpr T is_pow2(T x) noexcept
{
    return x && ((x & (x - 1)) == 0);
}
