...
struct mat<4, 2, T>
{
    ...
    VX_FORCE_INLINE constexpr mat(
        ...
        : columns{ col_type(static_cast<scalar_type>(x1), static_cast<scalar_type>(y1)),
                   ...
                   col_type(static_cast<scalar_type>(x4), static_cast<scalar_type>(y4)) } {}
    ...
    template <typename T>
    friend VX_FORCE_INLINE constexpr vec<2, T> operator*(const mat<4, 2, T>& m, const vec<4, T>& v) noexcept
    {
        return vec<2, T>(
    ...
    template <typename T>
    friend VX_FORCE_INLINE constexpr vec<2, T> operator*(const vec<4, T>& v, const mat<4, 2, T>& m) noexcept
    {
        return multiply(m, v);
    }

    template <typename T>
    friend VX_FORCE_INLINE constexpr mat<2, 2, T> operator*(const mat<4, 2, T>& m1, const mat<2, 4, T>& m2) noexcept
    {
        return mat<2, 2, T>(
    ...
    template <typename T>
    friend VX_FORCE_INLINE constexpr mat<3, 2, T> operator*(const mat<4, 2, T>& m1, const mat<3, 4, T>& m2) noexcept
    {
        return mat<3, 2, T>(
            (m1.columns[0].x * m2.columns[0].x) + (m1.columns[1].x * m2.columns[0].y) + (m1.columns[2].x * m2.columns[0].z) + (m1.columns[3].x * m2.columns[0].w),
            (m1.columns[0].y * m2.columns[0].x) + (m1.columns[1].y * m2.columns[0].y) + (m1.columns[2].y * m2.columns[0].z) + (m1.columns[3].y * m2.columns[0].w),
    ...
    template <typename T>
    friend VX_FORCE_INLINE constexpr mat<4, 2, T> operator*(const mat<4, 2, T>& m1, const mat<4, 4, T>& m2) noexcept
    {
        return mat<4, 2, T>(
            (m1.columns[0].x * m2.columns[0].x) + (m1.columns[1].x * m2.columns[0].y) + (m1.columns[2].x * m2.columns[0].z) + (m1.columns[3].x * m2.columns[0].w),
            (m1.columns[0].y * m2.columns[0].x) + (m1.columns[1].y * m2.columns[0].y) + (m1.columns[2].y * m2.columns[0].z) + (m1.columns[3].y * m2.columns[0].w),
