...
template <typename C>
struct formatter<fmt_test_types::point, C> : basic_format_spec<C>
{
private:
...
public:
    ...
    constexpr bool parse(
        const basic_parse_context<C>& ctx) noexcept
    {
    ...
    constexpr format_error format(
        ...
        const fmt_test_types::point& value) const noexcept
    {
        ...
        {
            const auto result = strconv::to_string<int, C>(
                v,
                buffer + count,
                sizeof(buffer) / sizeof(C) - count,
                {});

            if (result.err != strconv::to_string_error::none)
            {
                return false;
            }
        ...
        if (!append_int(value.y))
        {
            return format_error::invalid_argument;
        }

        buffer[count++] = C('}');
        return ctx.write_padded(*this, buffer, count, alignment::right);
    }
};
    ...
    const Tuple& args)
{
    ...
    else
    {
        ...
        for (size_t n = 0; n <= correct.size() + extra_chars; ++n)
        {
            ...
            const size_t suffix_size = buf_suffix + (space - n);
            ...
            if (n < correct.size())
            {
                ...
                VX_CHECK(all_of(last, suffix_size, fill_char));
            }
            else
            {
...
void test_common_cases()
{
    ...
    VX_SECTION("custom point formatter")
    {
        constexpr format_test_case<C, fmt_test_types::point> point_cases[] = {

            // Default formatter
            { LIT("{}"),      LIT("{1,2}"),      fmt::format_error::none,           fmt_test_types::point{ 1, 2 }     },
            { LIT("{}"),      LIT("{-5,8}"),     fmt::format_error::none,           fmt_test_types::point{ -5, 8 }    },

            // Explicit custom type
            { LIT("{:p}"),    LIT("{1,2}"),      fmt::format_error::none,           fmt_test_types::point{ 1, 2 }     },

            // Width and alignment
            { LIT("{:10}"),   LIT("     {1,2}"), fmt::format_error::none,           fmt_test_types::point{ 1, 2 }     },
            { LIT("{:<10}"),  LIT("{1,2}     "), fmt::format_error::none,           fmt_test_types::point{ 1, 2 }     },
            { LIT("{:>10}"),  LIT("     {1,2}"), fmt::format_error::none,           fmt_test_types::point{ 1, 2 }     },
            { LIT("{:^10}"),  LIT("  {1,2}   "), fmt::format_error::none,           fmt_test_types::point{ 1, 2 }     },
            ...
            // Negative values
            { LIT("{}"),      LIT("{-10,-20}"),  fmt::format_error::none,           fmt_test_types::point{ -10, -20 } },

            // Invalid formatter type
            { LIT("{:x}"),    LIT(""),           fmt::format_error::invalid_format, fmt_test_types::point{ 1, 2 }     },
            { LIT("{:d}"),    LIT(""),           fmt::format_error::invalid_format, fmt_test_types::point{ 1, 2 }     },
            { LIT("{:s}"),    LIT(""),           fmt::format_error::invalid_format, fmt_test_types::point{ 1, 2 }     },

            // Invalid format syntax
            { LIT("{:"),      LIT(""),           fmt::format_error::invalid_format, fmt_test_types::point{ 1, 2 }     },
            { LIT("{:.2}"),   LIT(""),           fmt::format_error::invalid_format, fmt_test_types::point{ 1, 2 }     },
            { LIT("{:+}"),    LIT(""),           fmt::format_error::invalid_format, fmt_test_types::point{ 1, 2 }     },
            { LIT("{:#}"),    LIT(""),           fmt::format_error::invalid_format, fmt_test_types::point{ 1, 2 }     },
            { LIT("{:04}"),   LIT(""),           fmt::format_error::invalid_format, fmt_test_types::point{ 1, 2 }     },
        };
