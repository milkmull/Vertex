...
VX_TEST_CASE(test_basic_thread)
{
    ...
    VX_CHECK(!t.join());
    VX_CHECK(t.get_id() == os::invalid_thread_id);
...
VX_TEST_CASE(test_thread_move_operations)
{
    ...
    const os::thread_id id = t1.get_id();
    VX_CHECK(id != os::invalid_thread_id);
    ...
    VX_CHECK(t1.get_id() == os::invalid_thread_id);
    ...
    VX_CHECK(t2.get_id() == os::invalid_thread_id);
...
{
    VX_CHECK(os::this_thread::get_id() != os::invalid_thread_id);
}
