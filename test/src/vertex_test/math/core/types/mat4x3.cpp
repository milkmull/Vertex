...
VX_TEST_CASE(test_mat4x3)
{
    ...
    VX_SECTION("constructors")
    {
        // implicit constructors
        {
            {
                const mat4x3 m;
                VX_CHECK_EQ(m.columns[0], vec3(0.0f, 0.0f, 0.0f));
                VX_CHECK_EQ(m.columns[1], vec3(0.0f, 0.0f, 0.0f));
                VX_CHECK_EQ(m.columns[2], vec3(0.0f, 0.0f, 0.0f));
                VX_CHECK_EQ(m.columns[3], vec3(0.0f, 0.0f, 0.0f));
            }

            {
                const mat4x3 m(mat4x3(2.0f));
                VX_CHECK_EQ(m.columns[0], vec3(2.0f, 2.0f, 2.0f));
                VX_CHECK_EQ(m.columns[1], vec3(2.0f, 2.0f, 2.0f));
                VX_CHECK_EQ(m.columns[2], vec3(2.0f, 2.0f, 2.0f));
                VX_CHECK_EQ(m.columns[3], vec3(2.0f, 2.0f, 2.0f));
            }
        }
        ...
        // explicit constructors
        {
            ...
            const vec3 c4(10.0f, 11.0f, 12.0f);

            {
                ...
                VX_CHECK_EQ(m.columns[3], vec3(10.0f, 11.0f, 12.0f));
            }

            {
                const mat4x3 m(c1, c2, c3, c4);
        ...
        // conversion constructors
        {
            ...
            {
                const mat4x3 m(true, 2u, 3.0, 4, 5.0f, 6u, 7, 8.0f, 9u, 10, 11.0f, 12u);
                VX_CHECK_EQ(m.columns[0], vec3(1.0f, 2.0f, 3.0f));
                VX_CHECK_EQ(m.columns[1], vec3(4.0f, 5.0f, 6.0f));
                VX_CHECK_EQ(m.columns[2], vec3(7.0f, 8.0f, 9.0f));
                VX_CHECK_EQ(m.columns[3], vec3(10.0f, 11.0f, 12.0f));
            ...
            {
                const mat4x3 m(vec3i(1, 2, 3), vec3u(4u, 5u, 6u), vec3(7.0f, 8.0f, 9.0f), vec3d(10.0, 11.0, 12.0));
                VX_CHECK_EQ(m.columns[0], vec3(1.0f, 2.0f, 3.0f));
                VX_CHECK_EQ(m.columns[1], vec3(4.0f, 5.0f, 6.0f));
                VX_CHECK_EQ(m.columns[2], vec3(7.0f, 8.0f, 9.0f));
                VX_CHECK_EQ(m.columns[3], vec3(10.0f, 11.0f, 12.0f));
        ...
        {
            {
                ...
                VX_CHECK_EQ(m.columns[3], vec3(13.0f, 14.0f, 15.0f));
            }
        }
    }
    ...
    VX_SECTION("index")
    {
        VX_CHECK(m43[0] == vec3(1.0f, 2.0f, 3.0f));
        VX_CHECK(m43[1] == vec3(4.0f, 5.0f, 6.0f));
        VX_CHECK(m43[2] == vec3(7.0f, 8.0f, 9.0f));
        VX_CHECK(m43[3] == vec3(10.0f, 11.0f, 12.0f));
    ...
    {
        VX_CHECK(m43 == m43);
        VX_CHECK(m43 != mat4x3(1.0f, -2.0f, 3.0f, 4.0f, -5.0f, 6.0f, 7.0f, -8.0f, 9.0f, 10.0f, -11.0f, 12.0f));
    }

    VX_SECTION("unary const operators")
    {
        VX_CHECK(+mat4x3(1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 12.0f) ==
    ...
    {
        VX_CHECK(++mat4x3(1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 12.0f) == mat4x3(2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 12.0f, 13.0f));
        VX_CHECK(--mat4x3(1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 12.0f) == mat4x3(0.0f, 1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f));
    ...
    VX_SECTION("binary arithmetic operators")
    {
        // addition
        {
            VX_CHECK(m43 + 5.0f == mat4x3(6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 12.0f, 13.0f, 14.0f, 15.0f, 16.0f, 17.0f));
            VX_CHECK(5.0f + m43 == mat4x3(6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 12.0f, 13.0f, 14.0f, 15.0f, 16.0f, 17.0f));
            VX_CHECK(m43 + mat4x3(5.0f) == mat4x3(6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 12.0f, 13.0f, 14.0f, 15.0f, 16.0f, 17.0f));
            VX_CHECK(mat4x3(5.0f) + m43 == mat4x3(6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 12.0f, 13.0f, 14.0f, 15.0f, 16.0f, 17.0f));
        }

        // subtraction
        {
            VX_CHECK(mat4x3(6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 12.0f, 13.0f, 14.0f, 15.0f, 16.0f, 17.0f) - 5.0f == m43);
            VX_CHECK(5.0f - mat4x3(6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 12.0f, 13.0f, 14.0f, 15.0f, 16.0f, 17.0f) == -m43);
            VX_CHECK(mat4x3(6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 12.0f, 13.0f, 14.0f, 15.0f, 16.0f, 17.0f) - mat4x3(5.0f) == m43);
            VX_CHECK(mat4x3(5.0f) - mat4x3(6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 12.0f, 13.0f, 14.0f, 15.0f, 16.0f, 17.0f) == -m43);
        }

        // multiplication
        {
            VX_CHECK(m43 * 5.0f == mat4x3(5.0f, 10.0f, 15.0f, 20.0f, 25.0f, 30.0f, 35.0f, 40.0f, 45.0f, 50.0f, 55.0f, 60.0f));
            VX_CHECK(5.0f * m43 == mat4x3(5.0f, 10.0f, 15.0f, 20.0f, 25.0f, 30.0f, 35.0f, 40.0f, 45.0f, 50.0f, 55.0f, 60.0f));

            // 3x1 * 4x3
            const vec3 m31(1.0f, 2.0f, 3.0f);
            VX_CHECK((m31 * m43) == vec4(14.0f, 32.0f, 50.0f, 68.0f));

            // 4x3 * 1x4
            const vec4 m14(1.0f, 2.0f, 3.0f, 4.0f);
            VX_CHECK((m43 * m14) == vec3(70.0f, 80.0f, 90.0f));

            // 4x3 * 2x4
            const mat2x4 m24(1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f);
            VX_CHECK((m43 * m24) == mat2x3(70.0f, 80.0f, 90.0f, 158.0f, 184.0f, 210.0f));

            // 4x3 * 3x4
            const mat3x4 m34(1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 12.0f);
            VX_CHECK((m43 * m34) == mat3x3(70.0f, 80.0f, 90.0f, 158.0f, 184.0f, 210.0f, 246.0f, 288.0f, 330.0f));

            // 4x3 * 4x4
            const mat4x4 m44(1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f, 12.0f, 13.0f, 14.0f, 15.0f, 16.0f);
            VX_CHECK((m43 * m44) == mat4x3(70.0f, 80.0f, 90.0f, 158.0f, 184.0f, 210.0f, 246.0f, 288.0f, 330.0f, 334.0f, 392.0f, 450.0f));
        }

        // division
        {
            VX_CHECK(mat4x3(5.0f, 10.0f, 15.0f, 20.0f, 25.0f, 30.0f, 35.0f, 40.0f, 45.0f, 50.0f, 55.0f, 60.0f) / 5.0f == m43);
            VX_CHECK(27720.0f / mat4x3(27720.0f, 13860.0f, 9240.0f, 6930.0f, 5544.0f, 4620.0f, 3960.0f, 3465.0f, 3080.0f, 2772.0f, 2520.0f, 2310.0f) == m43);
        }
    }
