...
VX_TEST_CASE(test_mat2x3)
{
    ...
    VX_SECTION("constructors")
    {
        ...
        // explicit constructors
        {
            ...
            {
                const mat2x3 m(c1[0], c1[1], c1[2], c2[0], c2[1], c2[2]);
                VX_CHECK_EQ(m.columns[0], c1);
                VX_CHECK_EQ(m.columns[1], c2);
            }

            {
                const mat2x3 m(c1, c2);
                VX_CHECK_EQ(m.columns[0], c1);
                VX_CHECK_EQ(m.columns[1], c2);
            }
        }
        ...
        {
            {
                const mat2x3 m(mat2x3(2u));
                VX_CHECK_EQ(m.columns[0], vec3(2.0f, 2.0f, 2.0f));
                VX_CHECK_EQ(m.columns[1], vec3(2.0f, 2.0f, 2.0f));
            }

            {
                const mat2x3 m(true, 2.0f, 3u, 4.0, 5, 6.0f);
                VX_CHECK_EQ(m.columns[0], vec3(1.0f, 2.0f, 3.0f));
                VX_CHECK_EQ(m.columns[1], vec3(4.0f, 5.0f, 6.0f));
            }

            {
                const mat2x3 m(vec3i(1, 2, 3), vec3u(4u, 5u, 6u));
                VX_CHECK_EQ(m.columns[0], vec3(1.0f, 2.0f, 3.0f));
                VX_CHECK_EQ(m.columns[1], vec3(4.0f, 5.0f, 6.0f));
            }
        }
    ...
    VX_SECTION("index")
    {
        VX_CHECK(m23[0] == vec3(1.0f, 2.0f, 3.0f));
        VX_CHECK(m23[1] == vec3(4.0f, 5.0f, 6.0f));
    ...
    {
        VX_CHECK(m23 == m23);
        VX_CHECK(m23 != mat2x3(-1.0f, 2.0f, -3.0f, 4.0f, -5.0f, 6.0f));
    }

    VX_SECTION("unary const operators")
    {
        VX_CHECK(+mat2x3f(1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f) == mat2x3f(1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f));
        VX_CHECK(+mat2x3f(1.0f, -2.0f, 3.0f, -4.0f, 5.0f, -6.0f) == mat2x3f(1.0f, -2.0f, 3.0f, -4.0f, 5.0f, -6.0f));
    ...
    {
        VX_CHECK(++mat2x3(1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f) == mat2x3(2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f));
        VX_CHECK(--mat2x3(1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f) == mat2x3(0.0f, 1.0f, 2.0f, 3.0f, 4.0f, 5.0f));
    ...
    VX_SECTION("binary arithmetic operators")
    {
        // addition
        {
            VX_CHECK(m23 + 5.0f == mat2x3(6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f));
            VX_CHECK(5.0f + m23 == mat2x3(6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f));
            VX_CHECK(m23 + mat2x3(5.0f) == mat2x3(6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f));
            VX_CHECK(mat2x3(5.0f) + m23 == mat2x3(6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f));
        }

        // subtraction
        {
            VX_CHECK(mat2x3(6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f) - 5.0f == m23);
            VX_CHECK(5.0f - mat2x3(6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f) == -m23);
            VX_CHECK(mat2x3(6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f) - mat2x3(5.0f) == m23);
            VX_CHECK(mat2x3(5.0f) - mat2x3(6.0f, 7.0f, 8.0f, 9.0f, 10.0f, 11.0f) == -m23);
        }

        // multiplication
        {
            VX_CHECK(m23 * 5.0f == mat2x3(5.0f, 10.0f, 15.0f, 20.0f, 25.0f, 30.0f));
            VX_CHECK(5.0f * m23 == mat2x3(5.0f, 10.0f, 15.0f, 20.0f, 25.0f, 30.0f));

            // 3x1 * 2x3
            const vec3 m31(1.0f, 2.0f, 3.0f);
            VX_CHECK((m31 * m23) == vec2(14.0f, 32.0f));

            // 2x3 * 1x2
            const vec2 m12(1.0f, 2.0f);
            VX_CHECK((m23 * m12) == vec3(9.0f, 12.0f, 15.0f));

            // 2x3 * 2x2
            const mat2x2 m22(1.0f, 2.0f, 3.0f, 4.0f);
            VX_CHECK((m23 * m22) == mat2x3(9.0f, 12.0f, 15.0f, 19.0f, 26.0f, 33.0f));

            // 2x3 * 3x2
            const mat3x2 m32(1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f);
            VX_CHECK((m23 * m32) == mat3x3(9.0f, 12.0f, 15.0f, 19.0f, 26.0f, 33.0f, 29.0f, 40.0f, 51.0f));

            // 2x3 * 4x2
            const mat4x2 m42(1.0f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f);
            VX_CHECK((m23 * m42) == mat4x3(9.0f, 12.0f, 15.0f, 19.0f, 26.0f, 33.0f, 29.0f, 40.0f, 51.0f, 39.0f, 54.0f, 69.0f));
        }

        // division
        {
            VX_CHECK(mat2x3(5.0f, 10.0f, 15.0f, 20.0f, 25.0f, 30.0f) / 5.0f == m23);
            VX_CHECK(60.0f / mat2x3(60.0f, 30.0f, 20.0f, 15.0f, 12.0f, 10.0f) == m23);
        }
    }
